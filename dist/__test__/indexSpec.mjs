...
var shopify = shopifyApi(shopifyEnv);
...
var sdk2 = __require("node-appwrite");
...
import { plainToClass as plainToClass3 } from "class-transformer";
...
var ProductService = class {
  ...
  async createProduct(productData) {
    ...
    const newProduct = plainToClass3(ProductDto, productData);
    return await ProductRepository.createAndSave(newProduct);
  }
  async updateProduct(id, productData) {
    const updatedProduct = plainToClass3(ProductDto, productData);
  ...
  async _genProductId(sellerId, productName) {
    return await bcrypt2.hashSync(sellerId + productName.toLowerCase(), 10);
  }
...
// src/__test__/indexSpec.ts
var request = supertest(src_default);
...
//# sourceMappingURL=indexSpec.mjs.map